      ...
      <Flex key={colorOptions} asChild gap="2" position="relative" width="32px" height="32px">
        <Text as="label" size="2">
          <Radio
            name={colorOptions}
            value={colorOptions}
            onClick={() => handleChange(colorOptions)}
            defaultChecked
            className="hidden" 
          />
          <div
            style={{ backgroundColor: colorOptions, borderRadius: '8px'  }}
            className={`absolute flex top-0 left-0 border-2 border-gray-500 z-10 w-[32px] h-[32px] bg-${colorOptions}-500 pointer-events-none`}
